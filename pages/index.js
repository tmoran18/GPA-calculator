...
export default function IndexPage() {
  ...
  const [gradesVsUnitsAlert, setGradesVsUnitsAlert] = useState(false);
  ...
  //TODO
  const calculateGPA = () => {
    // If totalUnits input is empty, throw alert else check units vs grades, calc GPA
    totalUnits == ""
      ? alert("Total number of units must not be empty.")
      : checkTotalUnitsVsTotalGrades();
  };

  const checkTotalUnitsVsTotalGrades = () => {
    totalGrades == totalUnits ? getGPA() : setGradesVsUnitsAlert(true);
  };
    ...
    <div className="h-screen bg-hero-pattern bg-no-repeat bg-cover p-15">
      <div className="p-5 ">
        ...
        <section className="p-5 md:p-10 lg:p-10 xl:p-15 rounded m-auto max-w-screen-sm bg-gray-300">
          ...
          {gradesVsUnitsAlert && (
            <div
              class="bg-red-100 border border-red-400 text-red-700 px-4 py-5 rounded relative text-center sm:text-left sm:py-4"
              role="alert"
            >
              <span class="block sm:inline">
                Number of units does not match number of grades.
              </span>
              <span
                onClick={closeAlert}
                class="absolute top-0 bottom-0 right-0 sm:px-4 sm:py-4"
              >
                <svg
